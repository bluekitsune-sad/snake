export function floodFillSearch(start, goal, cols, rows, snakeBody, onVisit) {
  // A BFS-like flood fill that doesn't prioritize goal; returns when goal reached
  const queue = [start];
  ...
  while (queue.length > 0) {
    ...
    for (const neighbor of getNeighbors(current, cols, rows, snakeBody)) {
